...
import edu.njrs.common.util.MenuSession;
...
import edu.njrs.manager.service.AuthorityServiceInterface;
...
public class AuthorityController {
    ...
    public ModelAndView initAuthority(PageBean pageBean, HttpServletRequest request, Authority authority) {
        ...
        // 查询管理员权限
        return new ModelAndView("manager/managerList").addObject("pageBean", authorityServiceInterface.queryAuthoerityList(authority, pageBean, createId));
    }
     ...
     *
     * @param authorid 角色id
     * @return 返回0 可以删除 1不可以删除
     */
    @RequestMapping("/isauthcandelete")
    @ResponseBody
    public int isAuthCanDelete(String[] authorid) {
        return authorityServiceInterface.isAuthCanDelete(authorid);
    }

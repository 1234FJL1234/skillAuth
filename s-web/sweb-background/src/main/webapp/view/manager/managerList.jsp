...
<%@ include file="/resources/common-tags.jsp" %>
<!DOCTYPE html>
<html lang="zh-cn">
  <head>
    <meta charset="utf-8">
            ...
            <div class="common_data p20">
              ...
              <table class="table table-striped table-hover">
                ...
                <tr>
                  ...
                  <td>  
                    <div class="btn-group">
                      <button type="button" class="btn btn-default" onclick="openEdit(${author.id });"><i class="glyphicon glyphicon-cog"></i>编辑</button>
				...
				<c:import url="../common/searchPag.jsp">
...
<input type="hidden" value="${managerFlag}" id="hi_managerId"/>
 ...
 function delauth(authId) {
     var url = 'isauthcandelete.htm?authorid=' + authId;
      // 判断是否可以删除
      $.ajax({
          type: 'post',
          url:url,
          ...
          success: function(data) {
              if (data > 0) {
                  showTipAlert('此角色已经关联用户不删除！');
              } else {
                  showDeleteOneConfirmAlert('deleteAuthority.htm?CSRFToken=${token}&authorid='+authId,'确定要删除此角色吗？');
              }
          }
      });
  }
  ...
  function batchAuthrity(){
      var chestr="";
      var checkboxs = $("input[name=authorid]");
      var oneSelect = false;
      ...
      var url = 'isauthcandelete.htm?authorid=' + chestr;
      // 判断是否可以删除
      $.ajax({
          type: 'post',
          url:url,
          ...
          success: function(data) {
              if (data > 0) {
                  showTipAlert('删除的角色中有角色已经关联用户不删除！');
              } else {
                  // 判断是否可以删除

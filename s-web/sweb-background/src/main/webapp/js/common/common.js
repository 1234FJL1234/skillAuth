...
$(function(){
    ...
    $('.menu_title').click(function(){
        var theIndex = $(this).parent().index('.menu_item');
        $('.menu_item:not('+theIndex+') .menu_body').slideUp('fast');
    ...
    /* 侧边栏手风琴效果 END */
    /* 为了自适应页面，协调相关内容的宽高 */
    $('.main_cont').css('height',$(window).height() - 50 + 'px');
 //   $('.sidebar').css('height',($(window).height() - 84) + 'px');
    $(window).resize(function(){
        $('.main_cont').css('hight',$(window).height() - 50 + 'px');
       // $('.sidebar').css('height',($(window).height() - 84) + 'px');
    });
...
// toast提示框
function zclipSwitch(target) {
    $('.' + target).fadeIn(function(){
        var t = setTimeout(function(){
            $('.toast-box:visible').fadeOut();
        }, 2000);
    });
}
...
//是否全部包含
Array.prototype.equals = function (array /*, index, last*/) {
 ...
 * @param url
 */
function toSubMenuPage(url){
    window.location.href = commonCtx+"/"+url;
}
function toSubMenuPage1(url,param){
    window.location.href = $("a[href*='"+url+"']").attr('href')+"&"+param;
}
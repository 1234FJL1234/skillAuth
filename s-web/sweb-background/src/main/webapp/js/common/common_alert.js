 ...
 * @param tip 提示内容
 */
function showTipAlert(tip,callback,isNotShowClose) {
    $("#modalDialog").remove();
    var dialogHtml =
    ...
    $('#modalDialog').modal({backdrop:'static',show:true});
}
...
function doAjaxDeleteOne(deleteUrl) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if(data==0){//存在一些审核通过的商铺
                showTipAlert("存在已经审核的店铺信息，无法删除！");
            }else{
                location.reload();
            }
        }
 ...
 * 简单的确认框，返回true或false
 */
function simpleConfirm(tips,ok,close) {
    $('#modalDialog').remove();
    var confirmDialog =
        '<div class="modal fade" id="modalDialog" tabindex="-1" role="dialog">'+
        '    <div class="modal-dialog">'+
